...
.DESCRIPTION
    Bootstraps 10_IMPORTS_RAW/regression_reports/[MMDDYYYY]/ with
    checklist, bug template, and triage worksheet.
...
# Resolve repo root (script lives at automation-bug-lifecycle/recreate-kit/scripts/)
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$KitRoot = Split-Path -Parent $ScriptDir
$ModuleRoot = Split-Path -Parent $KitRoot
$RepoRoot = Split-Path -Parent $ModuleRoot

$EvidenceRoot = Join-Path $RepoRoot "10_IMPORTS_RAW\regression_reports"
$TargetDir = Join-Path $EvidenceRoot $Date
$TemplatesDir = Join-Path $KitRoot "templates"
...
# Evidence README — customize placeholders
...
## Prompts (from recreate-kit/prompts/)
...
Write-Host "  2. Run triage prompt: automation-bug-lifecycle/recreate-kit/prompts/01-triage-regression-failure.md"
