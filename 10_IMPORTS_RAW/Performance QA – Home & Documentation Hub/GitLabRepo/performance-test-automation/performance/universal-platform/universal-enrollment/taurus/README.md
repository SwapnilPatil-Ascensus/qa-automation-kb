...
## Description
Run HTML based load testing for Universal Enrollment with blazemeter reporting

## Coverage
12/15 - Only first 2 pages of the Universal Enrollment are covered
3/6 - Update Univeral Enrollment to full flows (including some random data generation)
3/6 - Created local and grid executions
...
1. Run: bzt <script>
2. Example: bzt Blazemeter-UE-Local.yaml
...
##Generation data
1. From data/taurus-data-generation, run: mvn clean install
2. Generate a CSV from same directly for N rows: mvn compile exec:java -Dexec.args=<N>
