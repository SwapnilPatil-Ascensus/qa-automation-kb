# JIRA Stories — Unite MSC (Swapnil)

**Epic:** [QA-796](https://ascensuscollegesavings.atlassian.net/browse/QA-796) · **ENVP:** [QA-600](https://ascensuscollegesavings.atlassian.net/browse/QA-600)  
**Reporter:** Swapnil Patil

| # | Story | File | Suggested points |
|---|-------|------|------------------|
| 1 | Cross-team coordination + coverage matrix | [UNITE-MSC-coordination-coverage-matrix.md](./UNITE-MSC-coordination-coverage-matrix.md) | 5 |
| 2 | ENVP / DevOps pipeline integration | [UNITE-MSC-envp-devops-pipeline-integration.md](./UNITE-MSC-envp-devops-pipeline-integration.md) | 5 |
| 3 | Mobile 2 — all features validation | [UNITE-MSC-mobile2-all-features-validation.md](./UNITE-MSC-mobile2-all-features-validation.md) | 8 |
| 4 | Mobile 1 — all features validation | [UNITE-MSC-mobile1-all-features-validation.md](./UNITE-MSC-mobile1-all-features-validation.md) | 5 |

**References:** `programs/unite-msc/mobile-1/`, `programs/unite-msc/mobile-2/`, `programs/unite-msc/enrollment/`, `programs/government-savings-assessment/`
