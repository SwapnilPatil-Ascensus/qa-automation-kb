# Run all Mobile 1 + Mobile 2 QC4 suite profiles (Java 17, QC4 DB via qc4.properties)
...
$hostBak = "$hostFile.qc4run"
if (Test-Path $hostFile) { Rename-Item $hostFile $hostBak -Force }

$script:results = @()
$log = "$root\mobile\qc4-all-suites-results.log"
'' | Set-Content $log

function Run-Suite {
    param($Module, $Profile, $ReportEnv = 'QC4')
    $pom = if ($Module -eq 'mobile1') { "$root\mobile\mobile1\pom.xml" } else { "$root\mobile\mobile2\pom.xml" }
    $cmd = "mvn -f `"$pom`" test `"-P$Profile`" `"-Dmobile.ms.report.environment=$ReportEnv`""
    Write-Host "`n========== $Module : $Profile ==========" | Tee-Object -FilePath $log -Append
    ...
    if (-not $pass) { $out | Select-String -Pattern 'FAILURE|ERROR|No QAAUTOTEST|401|SSLHandshake' | Select-Object -First 5 | Tee-Object -FilePath $log -Append }
}

Run-Suite mobile1 acceptance-qc4
...
if (Test-Path $hostBak) { Rename-Item $hostBak $hostFile -Force }

"`n=== QC4 SUMMARY ===" | Tee-Object -FilePath $log -Append
$script:results | Format-Table -AutoSize | Out-String | Tee-Object -FilePath $log -Append
$script:results | Export-Csv "$root\mobile\qc4-all-suites-results.csv" -NoTypeInformation
