...
- **Scope:** Lightweight QC4 baseline using Postman (smoke) + JMeter (load), organized under `qa-automation-kb/programs/barcode-syn-443/`.
...
2. **Load assumptions** — rough peak scans/hour or concurrent Kofax sessions during busy season (draft profile in KB: `programs/barcode-syn-443/docs/03-load-profile.md`).
...
**KB folder:** `qa-automation-kb/programs/barcode-syn-443/` (story draft, Postman collection, environment strategy doc)
