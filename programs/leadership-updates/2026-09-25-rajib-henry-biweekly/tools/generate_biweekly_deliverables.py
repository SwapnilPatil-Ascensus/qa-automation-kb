...
def build_pptx(m: dict) -> Path:
    ...
    cards = [
        ...
        (PptInches(3.35), "Sprint 26.16 items", str(p["jira_sprint_items_current"]), TEAL, p["jira_sprint_items_note"]),
        (PptInches(6.05), "MSC coding", "100%", NAVY, p["msc_in_scope_note"]),
        (PptInches(8.75), "Stage1 health", "Recovered", CORAL, "Post DB-refresh / IDP co-work"),
        (PptInches(11.45), "ETA track", p["eta_delivery_rate"], GREEN, "Ahead of original plan"),
    ]
    ...
    slide_chart_page(prs, n := n + 1, "Sprint Delivery Focus",
        "Sprints 26.15 · 26.16 (current)", "02-sprint-focus-mix.png",
        [
            "Sprint 26.15: QA-1938 merged (!268); Bruno Postman migration started",
            "Sprint 26.16: MSC wrap-up (SharePoint, qTest, QC4) + V2/V3 gap execution",
            "Perf: M2 done · M1 done by Sep 26 · Enrollment remaining · Jenkins jobs ready",
            "V2→V3: Venkatesh enroll/login/reg almost done; member flows migrating to V3",
    ...
    slide_chart_page(prs, n := n + 1, "Enrollment — Endpoint Coverage",
        ...
        [
            ...
            "MR !268 (QA-1938) merged Sep 11 · Bruno M1/M2/Enrollment merged Sep 21 (!271)",
            f"3 deferred partner endpoints: {', '.join(enr.get('deferred', []))}",
            "Wrap-up: SharePoint KT, qTest manual cases, QC4 E2E proof — end Sprint 26.16",
    ...
    slide_bullets(slide, "Venkatesh — V2", [
        ...
        ts.get("venkatesh_v2", {}).get("member_migration", ""),
        ts.get("venkatesh_v2", {}).get("end_of_sprint", ""),
    ], PptInches(7.0), PptInches(1.45), PptInches(5.7), PptInches(2.5), NAVY, font_size=8.5)
    slide_bullets(slide, "He — V3 · Dinesh — MSC wrap-up", [
        f"He: {ts.get('he_v3', {}).get('note', '')}",
        f"He: {ts.get('he_v3', {}).get('done', '')} · {ts.get('he_v3', {}).get('in_progress', '')}",
        f"Dinesh: {ts.get('dinesh_msc', {}).get('coding', '')}",
        f"Target: {ts.get('dinesh_msc', {}).get('target', '')}",
    ], MARGIN, PptInches(4.15), PptInches(12.1), PptInches(2.4), TEAL, font_size=8.5)
    slide_footer(slide, ts.get("env_health", {}).get("context", "Stage1 recovered")[:140], n := n + 1)

    # 5c API suite ask
    ask = m.get("api_suite_ask", {})
    if ask:
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        slide_bg(slide)
    ...
    slide_bullets(slide, "Regression bugs — Sprints 26.15–26.16", bug_bullets,
    ...
    slide_chart_page(prs, n := n + 1, "Capacity — MSC Wrap-up vs New Work",
        "MSC wrap-up end 26.16 · capacity opens for API suites / Atlas / V2-V3", "03-capacity-timeline.png",
        [
            cap["msc_wrap"],
...
def build_docx(m: dict) -> Path:
    ...
    add_table(doc, ["Metric", "Value", "Notes"], [
        ("Sprint scope", p["sprint_scope"], "26.15 · 26.16"),
        ("GitLab merges", p["gitlab_merges"], p["gitlab_merges_note"]),
        ("Sprint 26.16 board items", p["jira_sprint_items_current"], p["jira_sprint_items_note"]),
