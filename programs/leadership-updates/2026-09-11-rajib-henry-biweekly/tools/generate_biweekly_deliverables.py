...
def chart_msc_progress(m: dict) -> Path:
    ...
    ax.set_title("Unite MSC — Program Progress (Sprint 26.15)", fontweight="bold", color="#003241")
...
def chart_sprint_mix(m: dict) -> Path:
    ...
    ax.bar(x - w, mix["sprint_26_14"], w, label="Sprint 26.14", color="#003241")
    ax.bar(x, mix["sprint_26_15"], w, label="Sprint 26.15 (current)", color="#00A3E0")
    ax.bar(x + w, mix["sprint_26_15_plan"], w, label="Next focus", color="#00B388")
    ax.set_xticks(x, labels, rotation=15, ha="right")
    ax.set_ylabel("Relative effort %")
    ax.set_title("Sprint Focus — 26.14 · 26.15 (current)", fontweight="bold")
...
def build_pptx(m: dict) -> Path:
    ...
    cards = [
        ...
        (PptInches(3.35), "Sprint 26.15 items", str(p["jira_sprint_items_current"]), TEAL, p["jira_sprint_items_note"]),
    ...
    slide_bullets(slide, "What leadership should know", [
        f"Scope: Sprints 26.14 and 26.15 (current) — MSC API coding is done on main",
        f"M1/M2/Enrollment: 76/79 in-scope endpoints — 3 partner APIs deferred by design",
        f"Pending MR {pm.get('mr', '!265')}: suite wiring + negatives — blocked on Snyk gate",
        "Sign-off sprint in progress: docs, traceability matrix, qTest, Bruno, KT (QA-2039–2047)",
    ...
    slide_chart_page(prs, n := n + 1, "Sprint Delivery Focus",
        "Sprints 26.14 · 26.15 (current)", "02-sprint-focus-mix.png",
        [
            "Sprint 26.14: Enrollment E2E complete on main (MRs 255–262); contribution perf JMX",
            "Sprint 26.15: Sign-off docs, traceability, MR !265 merge, V2 gap analysis (Venkatesh)",
            "V2: 8 P0 gaps are wire existing automation to daily regression — not greenfield",
            "Perf: M1 auth/login JMeter scripts in progress (QA-1983–1986)",
    ...
    slide_chart_page(prs, n := n + 1, "Unite MSC — Program Status",
        ...
        [
            ...
            f"Enrollment: {enr['catalog']} catalog ({enr['pct']}%) — standard {enr['core_e2e']} + subsequent {enr.get('subsequent_e2e', '5/5')}",
            f"Deferred: {', '.join(enr.get('deferred', []))}",
            f"Wrap-up: {', '.join(enr.get('remaining_work', [])[:2])}…",
    ...
    slide_chart_page(prs, n := n + 1, "Enrollment — Endpoint Coverage",
        ...
        [
            f"{enr['test_classes']} TestNG classes on main — standard + subsequent E2E complete",
            "MR !265 (QA-1938): negatives, optional-step config, M1/M2 suite XML wiring",
            f"3 deferred partner endpoints: {', '.join(enr.get('deferred', []))}",
            "Sign-off: coverage matrix, qTest, Bruno, enrollment GitLab nightly",
            "KB: programs/unite-msc/api-test-automation/mappings/enrollment-endpoint-current-state.csv",
    ...
    if pm:
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        slide_bg(slide)
        slide_header(slide, f"Pending Merge — {pm.get('mr', '!265')}", pm.get("title", "")[:80], "GitLab")
        add_kpi_card(slide, MARGIN, PptInches(1.55), PptInches(2.8), PptInches(1.35), "Changes", str(pm.get("changes", 110)), CYAN, "Files in MR")
        add_kpi_card(slide, PptInches(3.6), PptInches(1.55), PptInches(2.8), PptInches(1.35), "Author", pm.get("author", "Dinesh"), TEAL, f"Jira: closes QA-1938")
        add_kpi_card(slide, PptInches(6.2), PptInches(1.55), PptInches(2.8), PptInches(1.35), "Status", "Open", CORAL, pm.get("blocker", "Snyk gate")[:45])
        slide_bullets(slide, "What this MR delivers", [
            pm.get("scope", ""),
            "30 M1 + 25 M2 TestNG suite XML updates (integration + regression)",
            "Enrollment gap closures: content, enrollment-started, recurring, allocation-funds + negatives",
            "Reviewers: " + ", ".join(pm.get("reviewers", [])),
        ], MARGIN, PptInches(3.15), PptInches(12.1), PptInches(3.2), NAVY, font_size=9)
        slide_footer(slide, pm.get("url", ""), n := n + 1)
    ...
    slide_bullets(slide, "Regression bugs — Sprints 26.14–26.15", bug_bullets,
                  MARGIN, PptInches(4.05), PptInches(12.1), PptInches(2.5), CORAL, font_size=8.5)
    slide_footer(slide, ui["maintenance_note"], n := n + 1)

    # 8 V2 gaps (Venkatesh)
    v2g = m.get("v2_regression_gaps", {})
    if v2g:
        slide = prs.slides.add_slide(prs.slide_layouts[6])
        slide_bg(slide)
        slide_header(slide, "V2 Regression — Gap Analysis", f"By {v2g.get('analyst', 'Venkatesh Mallela')}", "V2")
        slide_bullets(slide, "P0 — Turn on existing automation (@dailyrun)", v2g.get("p0_turn_on", [])[:5],
                      MARGIN, PptInches(1.5), PptInches(6.0), PptInches(4.8), CORAL, font_size=8)
        slide_bullets(slide, "P1 — Extend coverage · P2 — Build new", (
            v2g.get("p1_extend", [])[:3] + ["—", "P2: YTD bucket, financial history, failed txns, bank history…"]
        ), PptInches(6.8), PptInches(1.5), PptInches(5.9), PptInches(4.8), NAVY, font_size=8)
        slide_footer(slide, v2g.get("summary", "")[:120] + "…", n := n + 1)
    ...
    slide_bullets(slide, "Sprint deliverables", [f"{d['name']}: {d['status']}" for d in deliverables],
    ...
    slide_footer(slide, "Perf intake governance — see next slide", n := n + 1)

    # 10 Perf governance ask
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    slide_bg(slide)
    slide_header(slide, "Performance Governance — Leadership Ask", "Stop last-minute emergency intake", "Ask")
    slide_bullets(slide, "In progress (Sprint 26.15)", [
        "Perf intake template + SLAs by change size (QA-1965–1969)",
        "Confluence Performance Testing Hub governance guidance",
        "Train Dev/QA/PM stakeholders on standard intake process",
    ], MARGIN, PptInches(1.5), PptInches(5.9), PptInches(2.6), CORAL)
    slide_bullets(slide, "What we need from leadership", [
        "Mandate perf gate for IDP login, enrollment, contributions, withdrawals before release",
        "Route requests through SDLC intake — not last-minute before deploy",
        "Approve M1 perf script priority: NY, NMD, OKD (QA-1983–1986)",
    ], PptInches(6.7), PptInches(1.5), PptInches(5.9), PptInches(2.6), TEAL)
    slide_footer(slide, "Contribution JMX delivered Sprint 26.14 — M1 scripts in flight", n := n + 1)
    ...
    s.text_frame.paragraphs[0].text = "Detailed briefing document shared · KB: programs/leadership-updates/2026-09-11-rajib-henry-biweekly/"
    s.text_frame.paragraphs[0].font.size = PptPt(12)
    s.text_frame.paragraphs[0].font.color.rgb = TEAL

    out = DELIVERABLES / "AM-Squad-Biweekly-Status-Rajib-Henry-Sep11-2026.pptx"
...
def build_docx(m: dict) -> Path:
    ...
    rows_meta = [
        ...
        ("Companion deck", "AM-Squad-Biweekly-Status-Rajib-Henry-Sep11-2026.pptx"),
    ]
    ...
    pm = m.get("pending_merge", {})
    doc.add_paragraph(
        "Unite MSC API automation coding is complete on main: Mobile 1 (26/26), Mobile 2 (25/25), and "
        "Enrollment (25/28 catalog — 3 partner endpoints deferred). Standard + subsequent E2E flows are "
        f"on main; MR {pm.get('mr', '!265')} (QA-1938) adds suite wiring, negatives, and validation gaps — "
        "pending Snyk/security gate approval. Sprint 26.15 focus: sign-off documentation (QA-2039–2047), "
        "traceability matrix, and Venkatesh V2 regression gap analysis. Post-MSC capacity can pivot to "
        "Atlas (Oct) or V2 P0 daily-regression wiring per leadership priority."
    )
    p = m["pulse"]
    u = m["unite_msc"]
    add_table(doc, ["Metric", "Value", "Notes"], [
        ("Sprint scope", p["sprint_scope"], "26.14 · 26.15"),
        ("GitLab merges", p["gitlab_merges"], p["gitlab_merges_note"]),
        ("Sprint 26.15 board items", p["jira_sprint_items_current"], p["jira_sprint_items_note"]),
    ...
    if pm:
        doc.add_heading(f"Pending merge {pm.get('mr', '!265')}", level=2)
        for item in [
            f"Title: {pm.get('title', '')}",
            f"Author: {pm.get('author', '')} · Status: {pm.get('status', '')}",
            f"Changes: {pm.get('changes', 110)} files · Blocker: {pm.get('blocker', '')}",
            pm.get("scope", ""),
            pm.get("url", ""),
        ]:
            doc.add_paragraph(item, style="List Bullet")
    doc.add_heading("Sprint 26.14–26.15 MSC highlights (Jira)", level=2)
    highlights = [
        "MR 255–262 merged — enrollment submit, subsequent E2E, standard enrollment (QA-1404, QA-1939)",
        "QA-1938 — gap inventory + negatives in MR !265 (Review)",
        "QA-2039–2047 — sign-off docs, README index, troubleshooting, AI scenario guide",
        "QA-2048–2053 — legacy-to-TestNG traceability matrix",
        "QA-1405 — Mobile 2 GitLab nightly delivered; enrollment nightly in wrap-up",
    ]
    for h in highlights:
    ...
    doc.add_heading("Sprint 26.14–26.15 highlights", level=2)
    for item in ui["v3_highlights"] + ui["v2_highlights"]:
        doc.add_paragraph(item, style="List Bullet")
    v2g = m.get("v2_regression_gaps", {})
    if v2g:
        doc.add_heading(f"V2 regression gaps ({v2g.get('analyst', 'Venkatesh')})", level=2)
        doc.add_paragraph(v2g.get("summary", ""))
        doc.add_heading("P0 — turn on existing automation", level=3)
        for item in v2g.get("p0_turn_on", []):
            doc.add_paragraph(item, style="List Bullet")
        doc.add_heading("P1 — extend coverage", level=3)
        for item in v2g.get("p1_extend", []):
            doc.add_paragraph(item, style="List Bullet")
        doc.add_heading("P2 — build new automation", level=3)
        for item in v2g.get("p2_build_new", []):
            doc.add_paragraph(item, style="List Bullet")
    doc.add_heading("V2 → V3 migration — discussion needed", level=2)
    doc.add_paragraph(ui["migration_discussion"])
    doc.add_heading("Regression bugs (Sprints 26.14–26.15)", level=2)
    ...
    doc.add_heading("Deliverables (Sprints 26.14–26.15)", level=2)
    ...
    out = DELIVERABLES / "AM-Squad-Biweekly-Status-Rajib-Henry-Sep11-2026.docx"
    try:
        doc.save(out)
    except PermissionError:
        out = DELIVERABLES / "AM-Squad-Biweekly-Status-Rajib-Henry-Sep11-2026-UPDATED.docx"
        doc.save(out)
