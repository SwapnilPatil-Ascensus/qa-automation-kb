...
def chart_msc_progress(m: dict) -> Path:
    ...
    ax.set_title("Unite MSC — Program Progress (Sprint 26.13)", fontweight="bold", color="#003241")
...
def chart_sprint_mix(m: dict) -> Path:
    ...
    ax.bar(x - w, mix["sprint_26_11"], w, label="Sprint 26.11", color="#003241")
    ax.bar(x, mix["sprint_26_12"], w, label="Sprint 26.12", color="#00A3E0")
    ax.bar(x + w, mix["sprint_26_13"], w, label="Sprint 26.13 (current)", color="#00B388")
    ax.set_xticks(x, labels, rotation=15, ha="right")
    ax.set_ylabel("Relative effort %")
    ax.set_title("Sprint Focus — 26.11 · 26.12 · 26.13", fontweight="bold")
...
def chart_capacity_timeline(m: dict) -> Path:
    phases = ["Now\n(Sprint 26.13)", "Sprint close\n(26.13 end)", "Q3 Close", "Q4+"]
    msc = [95, 100, 100, 100]
    other = [25, 50, 65, 80]
...
def build_pptx(m: dict) -> Path:
    ...
    cards = [
        ...
        (PptInches(3.35), "Sprint 26.13 items", str(p["jira_sprint_items_current"]), TEAL, p["jira_sprint_items_note"]),
    ...
    slide_bullets(slide, "What leadership should know", [
        "Scope: Sprints 26.11, 26.12, and 26.13 (current) — not Apr–Aug historical totals",
        "Mobile 1 and Mobile 2 are 100% of in-scope business endpoints; enrollment is the active build track",
        "Team delivers across MSC API, V2/V3 UI, performance, pipeline, and standards in parallel",
    ...
    slide_chart_page(prs, n := n + 1, "Sprint Delivery Focus",
        "Sprints 26.11 · 26.12 · 26.13 (current)", "02-sprint-focus-mix.png",
        [
            "Sprint 26.11: Mobile 1/2 API completion, nightly regression ops, perf MSC login",
            "Sprint 26.12: MSC sign-off package, enrollment kickoff, emergency perf (barcode + Jahia)",
            "Sprint 26.13: Enrollment API slices (QA-1595–1603), GitLab nightly, M1/M2 sign-off prep",
            "V2/V3 nightly regression and triage continue in parallel every sprint",
        ], "Relative effort index — not FTE hours", "Delivery", panel_h=PptInches(5.5))

    # 4 MSC
    m1 = u["mobile1"]
    slide_chart_page(prs, n := n + 1, "Unite MSC — Program Status",
        "Mobile 2 100% · Mobile 1 100% · Enrollment ~30%", "01-msc-progress.png",
        [
            f"Mobile 2: {u['mobile2']['automated']}/{u['mobile2']['total']} endpoints — ready for leadership sign-off",
            f"Mobile 1: {m1['automated']}/{m1['total']} in-scope operations — 100% coded and suite-routed*",
            f"Enrollment: ~{u['enrollment']['pct']}% — first API vertical slices in Sprint 26.13",
            f"Target: program complete {u['target_completion']}",
            "*Regression, integration, or smoke per endpoint; docs/health routes excluded",
        ], u["footnote"], "MSC", panel_h=PptInches(5.55))

    # 5 MSC coverage chart
    ...
    # 10 Capacity
    slide_chart_page(prs, n := n + 1, "Capacity Opening Up",
        "MSC wraps Sprint 26.13 — team needs next priorities", "03-capacity-timeline.png",
        [
            "1–2 resources free after MSC sign-off (target: end of Sprint 26.13)",
            "Dinesh → enrollment completion · Sunil → MSC manual test case package",
            "Wence/Venkatesh → V2 sunset / CSR enrollment migration to V3 on GitLab",
            "We need a 2-quarter roadmap to plan epics and Jira stories now",
        ], "Without roadmap we cannot commit ETAs for Q4", "Capacity")
    ...
    slide_bullets(slide, "AHA summary (Brian Danilczyk)", [
        ...
        "Scope: validate even traffic distribution after Redis session externalization; load + perf testing",
        "Success: eliminate hotspot nodes; dynamic routing vs IP-hash sticky sessions",
    ...
    s.text_frame.paragraphs[0].text = "Detailed briefing document shared · KB: programs/leadership-updates/2026-08-14-rajib-henry-biweekly/"
    s.text_frame.paragraphs[0].font.size = PptPt(12)
    s.text_frame.paragraphs[0].font.color.rgb = TEAL

    out = DELIVERABLES / "AM-Squad-Biweekly-Status-Rajib-Henry-Aug2026.pptx"
...
def build_docx(m: dict) -> Path:
    ...
    rows_meta = [
        ...
        ("Companion deck", "AM-Squad-Biweekly-Status-Rajib-Henry-Aug2026.pptx"),
    ]
    ...
    doc.add_paragraph(
        ...
        "performance testing, and pipeline/standards work. This briefing covers Sprints 26.11, 26.12, and "
        "26.13 (current) only — not Apr–Aug historical totals. Mobile 1 and Mobile 2 are 100% of in-scope "
        "business endpoints (26 + 25); enrollment is ~30% with first API slices in Sprint 26.13. The team "
        "expects to close the MSC program by end of Sprint 26.13 and will have 1–2 resources available for "
        "new priorities — but we need a two-quarter roadmap and sign-off owners to plan effectively."
    )
    p = m["pulse"]
    u = m["unite_msc"]
    add_table(doc, ["Metric", "Value", "Notes"], [
        ("Sprint scope", p["sprint_scope"], "26.11 · 26.12 · 26.13"),
        ("GitLab merges", p["gitlab_merges"], p["gitlab_merges_note"]),
        ("Sprint 26.13 board items", p["jira_sprint_items_current"], p["jira_sprint_items_note"]),
        ...
        ("MSC Enrollment", f"~{u['enrollment']['pct']}%", u["enrollment"]["status"]),
    ...
    add_table(doc, ["Module", "Coverage", "Status"], [
        ...
        ("Enrollment", "~30%", "Framework + first API slices in Sprint 26.13 (QA-1595–1603)"),
    ])
    ...
    doc.add_heading("Sprint 26.13 MSC highlights (Jira)", level=2)
    highlights = [
        "QA-1595–1603 — Enrollment API automation vertical slices (health, plans, prospect, wizard steps)",
        "QA-1405 / QA-1544–1549 — Mobile 2 GitLab nightly regression job and runbook",
        "QA-1668–1671 — Mobile 1/2 coverage validation and sign-off prep",
        "QA-1627–1631 — Enrollment prospect happy-path integration spike",
    ]
    for h in highlights:
    ...
    out = DELIVERABLES / "AM-Squad-Biweekly-Status-Rajib-Henry-Aug2026.docx"
    try:
        doc.save(out)
    except PermissionError:
        out = DELIVERABLES / "AM-Squad-Biweekly-Status-Rajib-Henry-Aug2026-UPDATED.docx"
        doc.save(out)
